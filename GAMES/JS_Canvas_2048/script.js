...
const cnvsSettings = {width: 400, height: 400, gameSize: 4, lineW: 2, generatedNumsEachMove:1};
...
window.addEventListener("keyup", function(event){
  if(!isGameOver){
    ...
    CTX.fillStyle = "white";
    CTX.fillRect(cnvsSettings.width/2-INTERVAL,cnvsSettings.height/2-INTERVAL/2, INTERVAL*2, INTERVAL);
...
function gameOverHandlerWhenDirectionButtonPressed(_buttonPressed){
  ...
  if(movementHandler.countAllDirectionsPressedForGameOverHandling(
      storeDirectionPressedAndPointsForGameOverHandling,
...
function moveSquareInSpecificPosition(theNumber, x, y){
    let nColor;
    if(theNumber === 0){nColor = "gray"}
    else if(theNumber <= 10 && theNumber > 0){nColor = "TURQUOISE"}
    else if (theNumber <= 50 && theNumber > 10){nColor = "#b3257c"}
    else if (theNumber <= 100 && theNumber > 50){nColor = "#820954"}
    else if (theNumber <= 1000 && theNumber > 100){nColor = "YELLOW"}
    else if (theNumber <= 4000 && theNumber > 1000){nColor = "LIME"}
    else if (theNumber <= 10000 && theNumber > 4000){nColor = "MAGENTA"}
    else if (theNumber <= 50000 && theNumber > 10000){nColor = "CYAN"}
    else if (theNumber <= 100000 && theNumber > 50000){nColor = "NAVY"}
    else{
        nColor = "#00ff33";
    }
...
function createAndDrawGirdOnCanvas(){
  ...
  CTX.beginPath();
  for (let x = 0; x < cnvsSettings.width; x+=cnvsSettings.width / cnvsSettings.gameSize) {
      // Start a new Path
      CTX.moveTo(0, 0);
      CTX.lineTo(0, x);
      CTX.moveTo(0, x);
  ...
  for (let y = 0; y < cnvsSettings.height; y+=cnvsSettings.width / cnvsSettings.gameSize) {
      // Start a new Path
      CTX.moveTo(0, 0);
      CTX.lineTo(y, 0);
      CTX.moveTo(y, 0);
