...
function threeInRowInArray(a) {
  // This function searches three+ in row in one sub array.
  let c = 1;
  let arrOfIndexes = [];
...
function generateNewObjectsAndRender(arrOfIndexes, row_OR_col_index, hor_OR_ver) {
  // Where could pass array of indexes and just generate something new :)
  //amazingList.push({n:index+1, c:randomColor});
  ...
  const addToGetCorrectIndex =
    hor_OR_ver === "hor" ? row_OR_col_index * CANDIES_IN_COL : row_OR_col_index;
