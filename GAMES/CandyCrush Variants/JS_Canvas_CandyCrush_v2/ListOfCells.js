class ListOfCells {
    ...
    for (let theRow = 0; theRow < this.listOfCells.length; theRow++) {
      for (let col = 0; col < this.listOfCells[0].length; col++) {
        ...
        try {
          ...
          displayText(
            `r-${element.arrayROW}:${element.arrayCOL}`,
            textX,
            textY + 7
          );
          // }
  ...
  testSetEqualCellColor(theArray) {
    ...
    for (let index = 0; index < this.listOfCells.length; index++) {
      for (let col = 0; col < this.listOfCells[0].length; col++) {
        ...
        if (
          c.arrayROW === testResult.rowMin &&
          c.arrayCOL >= testResult.colMin &&
          c.arrayCOL <= testResult.colMax
        ) {
          c.cellColor = "white";
        }
      }
      ...
      for (let col = this.listOfCells[0].length; col >= 0; col--) {
        ...
        try {
          ...
          if (false) {
            ...
            if (
              element.arrayROW <= testResult.rowMin &&
              element.arrayCOL >= testResult.colMin &&
              element.arrayCOL <= testResult.colMax
            ) {
              // element.update();
            ...
            // Verticaly
            if (
              element.arrayROW <= testResult.rowMax &&
              element.arrayCOL >= testResult.colMin &&
              element.arrayCOL <= testResult.colMax
            ) {
              // element.update();
              // element.cellColor = "white";
              element.updateBySomeAmount(
                possibleCellHW * (testResult.rowMax - testResult.rowMin + 1)
              );
            }
          }
  ...
  updateAllDown() {
    for (let row = 0; row < this.listOfCells.length; row++) {
      for (let col = 0; col < this.listOfCells[0].length; col++) {
  ...
  moveDownIfSomethingIsMissing() {
    for (let row = this.listOfCells.length - 1; row >= 0; row--) {
      for (let col = 0; col < 10; col++) {
        ...
        if (element) {
          // from 2048 GAME if element is not 0 it can be moved down // element !== 0
          ...
          for (let i = row; i < this.listOfCells.length - 1; i++) {
            ...
            if (!nextRow) {
              // from 2046 GAME if element in next cell below is not 0 all can be moved down
              rowIndex = i;
              break;
            }
          }
