...
function updateContent(cellId) {
  ...
  if (gameResult === false) {
    if (cell.innerHTML === "") {
      ...
      if (AIautoMove) {
        didComputerWin = AIMakesMove(gameResult);
      } // If This is win alert is still not triggered :(
      if (!AIautoMove) {
        if (gameResult !== false) {
...
function checkWineer(theGameField) {
  ...
  for (let row = 0; row < gameFLength; row++) {
    ...
    for (let col = 0; col < gameFLength; col++) {
      ...
      // ----------------------------------------
      if (
        cXwinByRows === gameFLength ||
        cXwinByCols === gameFLength ||
        cXwinByDiag === gameFLength ||
        cXwinByDiag_2 === gameFLength
      ) {
        gameResult = "X";
        break;
      } else if (
        cOwinByRows === gameFLength ||
        cOwinByCols === gameFLength ||
        cOwinByDiag === gameFLength ||
        cOwinByDiag_2 === gameFLength
      ) {
        gameResult = "O";
        break;
      }
